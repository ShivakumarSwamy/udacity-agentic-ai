...
from app.util import OpenAIModels, get_completion, openai_client
...
def parse_analysis_and_largest_spike(response):
    ...
    response_split = response.split("```json")
    analysis = response_split[0].strip()
    json_str = response_split[1].split("```")[0].strip()
...
def main():
    ...
    # Step-by-step results are better, compared to no explicit instruction.
    ...
    Instructions:
    * First, find all sales spikes for each product. 
    * For each product, identify the following: 
        * the date of the sales spike
        * the amount of the sales spike and percentage increase
    ...
    * Conclude with the single largest spike according to the percentage increase with a short explanation of it.
    ...
    analysis, largest_spike = parse_analysis_and_largest_spike(explicit_cot_response_1)
